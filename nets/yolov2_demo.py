...
class YOLO_TF:
    ...
    filewrite_txt = False
    disp_console = True
    weights_file = 'ckpt/yolo_hgx.ckpt'
    alpha = 0.1
    threshold = 0.2
    iou_threshold = 0.5
    num_class = 80
    ...
    def build_networks(self):
        ...
        #skip dropout_31
        self.layer29 = yolo.yolo_net(self.x)
    ...
    def detect_from_cvmat(self,img):
        ...
        net_output = self.sess.run(self.layer29,feed_dict=in_dict)
        net_output_one = net_output[0].reshape(self.grid_size,self.grid_size,5,85)
        print np.shape(net_output_one)
        self.result = self.interpret_output(net_output_one)
    ...
    def detect_from_crop_sample(self):
        ...
        for c in range(3):
            for y in range(416):
                for x in range(416):
                    inputs[0,y,x,c] = f[c*416*416+y*416+x]
    ...
    def interpret_output(self,output):
        probs = np.zeros((self.grid_size,self.grid_size,2,self.num_class))
        class_probs = np.reshape(output[0:13520],(self.grid_size,self.grid_size,self.num_class))
        scales = np.reshape(output[13520:14872],(self.grid_size,self.grid_size,2))
        boxes = np.reshape(output[14872:],(self.grid_size,grid_size,2,4))
        offset = np.transpose(np.reshape(np.array([np.arange(self.grid_size)]*14),(2,self.grid_size,self.grid_size)),(1,2,0))
